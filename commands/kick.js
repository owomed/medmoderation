...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!message.guild.me.hasPermission('KICK_MEMBERS')) {
            return message.reply('`Botun bu komudu kullanmak için gerekli izinlere sahip değil!`').then(x => x.delete({ timeout: 3000 }));
        }
        ...
        if (!üye || !sebep) {
            return message.reply('`Sunucudan atabilmek için üye ve sebep belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        }
        
        if (!sebep.trim()) {
            return message.reply('`Geçerli bir atılma sebebi belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        }
        ...
        try {
            ...
            await message.reply(embed).then(() => message.react(id.Emojiler.başarılıemojiid)).catch(err => {
            ...
            message.reply('`Kick işlemi sırasında bir hata oluştu!`').then(x => x.delete({ timeout: 9000 }), message.react(id.Emojiler.başarısızemojiid));
        }
    }
};
