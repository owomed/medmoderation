...
module.exports = {
    ...
    // Prefix komutu bilgisi (slash komutlarına ek olarak)
    name: 'kick',
    ...
    async execute(interactionOrMessage) {
        ...
        // Komutun prefix mi yoksa slash mı olduğunu kontrol et ve değişkenleri ata
        ...
        // Botun yetkisi kontrolü (Aynı kaldı)
        ...
        const sicilData = {
            Yetkili: author.id,
            Tip: "KICK", // Sicil tipi: KICK
            Sebep: reason,
            Zaman: Date.now()
        };

        try {
            // Kick işlemi (Discord API)
            await member.kick(reason);

            // Sicil modelini kullanarak kullanıcının sicil dizisine yeni kaydı ekle
            // Eski quick.db: db.push(`üye.${member.id}.sicil`, { ... });
            ...
            // ⭐️ MONGODB (Mongoose) SİCİL KAYDI BİTİŞ
