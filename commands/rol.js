...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!üye || !rol) return message.lineReply('`Lütfen geçerli bir üye ve rol belirtin!`').then(x => x.delete({ timeout: 3000 }));
        ...
        if (args[0] === "al") {
            ...
            üye.roles.remove(rol).then(() => {
                message.lineReply('`Başarıyla etiketlenen üyeden rolü aldım.`').then(x => x.delete({ timeout: 7 * 1000 }), message.react(id.Emojiler.başarılıemojiid));
            });
            return;
        };
        ...
        if (args[0] === "ver") {
            ...
            üye.roles.add(rol).then(() => {
                message.lineReply('`Başarıyla etiketlenen üyeye rol verdim.`').then(x => x.delete({ timeout: 7 * 1000 }), message.react(id.Emojiler.başarılıemojiid));
            });
            return;
        };
