...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        let süre = args[1];
        if (!üye || !süre || !sebep) return message.reply('`Jaile atmak için üye, süre(1s,1m,1h,1d) ve sebep belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        if (üye.roles.cache.get(id.Jail.jailrolid)) return message.reply('`Etiketlenen üye zaten jailde!`').then(x => x.delete({ timeout: 3000 }));
        ...
        üye.roles.set([id.Jail.jailrolid]), message.reply(`\`Etiketlenen üye başarıyla (${süre}) boyunca jaile atıldı!\``).then(x => x.delete({ timeout: 9000 }), message.react(id.Emojiler.başarılıemojiid))
