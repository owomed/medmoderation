...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!üye || !sebep) {
            return message.lineReply('`Jaile atabilmek için üye ve sebep belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        }
        if (üye.roles.cache.has(id.Jail.jailrolid)) {
            return message.lineReply('`Etiketlenen üye zaten jailde!`').then(x => x.delete({ timeout: 3000 }));
        }
        ...
        try {
            ...
            // Kanala mesaj gönder
            await message.lineReply(`\`${üye.user.tag}\` başarıyla jaile atıldı!`).then(x => x.delete({ timeout: 9000 }), message.react(id.Emojiler.başarılıemojiid));
            ...
            return message.lineReply('`Jail işlemi sırasında bir hata oluştu.`').then(x => x.delete({ timeout: 3000 }));
        }
    }
};
