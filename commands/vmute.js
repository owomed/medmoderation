...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!üye || !sebep) return message.reply('`Ses kanallarında susturmak için üye ve sebep belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        ...
        üye.voice.setMute(true), message.reply('`Etiketlenen üye başarıyla ses kanallarında susturuldu!`').then(x => x.delete({ timeout: 9000 }), message.react(id.Emojiler.başarılıemojiid))
    }
}
