...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!üye) return message.reply('`Ses kanallarında susturulmasını kaldırmak için bir kullanıcı belirtmelisin!`').then(x => x.delete({ timeout: 3000 }));
        ...
        üye.voice.setMute(false), message.reply('`Etiketlenen üyenin başarıyla ses kanallarında ki susturulması kaldırıldı!`').then(x => x.delete({ timeout: 9000 }), message.react(id.Emojiler.başarılıemojiid))
    }
}
